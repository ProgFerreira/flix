# Dispara POST /api/cron/process-videos com o CRON_SECRET do .env.
# Recupera uploads travados em processing (FFmpeg ausente, restart no meio).
# Uso: powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-process-videos-cron.ps1

$ErrorActionPreference = "Stop"
$Root = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$EnvFile = Join-Path $Root ".env"
$LogDir = Join-Path $Root "logs"
$LogFile = Join-Path $LogDir "process-videos-cron.log"
...
$base = $base.TrimEnd("/")
$url = "$base/api/cron/process-videos"
