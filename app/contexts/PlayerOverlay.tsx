...
import type { WatchVideo } from "@/app/contexts/PlayerContext"
...
export function PlayerOverlay({
  ...
  const [quality, setQuality] = useState(video.qualities?.includes("480") ? "480" : "auto")
  const iframeRef = useRef<HTMLIFrameElement | HTMLVideoElement>(null)
  const lastSaved = useRef(0)
  const resumeAt = useRef(video.startSeconds ?? 0)
  ...
  const skipPlaylist = useCallback((delta: number) => {
    ...
    if (!item) return
    persistProgress()
    onPlay({ ...item, playlist: { items: video.playlist.items, index } })
  }, [onPlay, persistProgress, video])
