...
test.describe.serial("Critical user journeys", () => {
  ...
  test("admin can assemble a course that subscribers can open", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: `Incluir ${video.title}` })).toBeVisible()
    await page.getByRole("button", { name: `Incluir ${video.title}` }).click()
    await expect(page.getByText(video.title, { exact: true })).toBeVisible()
    await page.getByRole("button", { name: "Adicionar aula" }).click()
    await expect(page.getByRole("button", { name: `Incluir ${uploadTitle}` })).toBeVisible()
    await page.getByRole("button", { name: `Incluir ${uploadTitle}` }).click()
